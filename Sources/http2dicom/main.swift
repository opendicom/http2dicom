...
let http2dicomLogger = Logger(label: "com.opendicom.com.http2dicom.main")
http2dicomLogger.info("main info")
...
/*
 ...
 let sslPrivateKey = try! NIOSSLPrivateKeySource.privateKey(
     NIOSSLPrivateKey(buffer: [Int8](samplePKCS8PemPrivateKey.utf8CString), format: .pem)
     { providePassword in providePassword("thisisagreatpassword".utf8) }
 )

 // Load the certificate
 let sslCertificate = try! NIOSSLCertificateSource.certificate(
     NIOSSLCertificate(buffer: [Int8](samplePemCert.utf8CString), format: .pem)
 )

 */
let sslPrivateKey = try! NIOSSLPrivateKeySource.privateKey(
    NIOSSLPrivateKey(bytes: (samplePKCS8PemPrivateKey.utf8CString).map{ x in UInt8(x) }, format: .pem)
    { providePassword in providePassword("thisisagreatpassword".utf8) }
)

// Load the certificate
let sslCertificate = try! NIOSSLCertificateSource.certificate(
    NIOSSLCertificate(bytes: (samplePemCert.utf8CString).map{ x in UInt8(x) }, format: .pem)
)
